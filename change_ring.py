...
def merge_ring(inputs, output, instance):
    ...
    for gpu in root1.findall('.//gpu'):
        ...
        ## 先获得其他tree中对应gpu的tb
        for root in roots:
            tree_gpus = root.findall('.//gpu')
            for gpu2 in tree_gpus:
...
if __name__ == "__main__":
    ...
    output = f"{dir_path}/merged_ring_{'_'.join([str(i) for i in base_ring])}/test.xml"
    merge_ring(inputs=inputs, output=output, instance=8)